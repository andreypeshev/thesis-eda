    ...
    for (cond_c in 1:n_distinct(data_prb$c)){
      for (cond_d in 1:n_distinct(data_prb$d)){
        for (cond_e in 1:n_distinct(data_prb$e)){
          for(cond_f in 1:n_distinct(data_prb$f)){
            for(cond_g in 1:n_distinct(data_prb$g)){
              count <- count + 1
            }  
          }
        }
        ...
        for (cond_e in 1:n_distinct(data_prb$e)){
          for (cond_f in 1:n_distinct(data_prb$f)){
            ...
            if (cond_a == 1 & cond_b != 8){
              ...
              #filtering the data 
              filt_data <- data_prb %>% filter(a==cond_a & b==cond_b & c==cond_c & d==cond_d & e==cond_e & f==cond_f)
              euc_data <- euclideanDistance(filt_data$x)
