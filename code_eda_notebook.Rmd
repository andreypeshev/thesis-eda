...
```{r}
data_prb %>% filter(a == 3 & c==3 & e==1) %>% ggplot(aes(x=g, y=x)) + geom_boxplot()
```
...
data_a %>% filter(b==8, c==3, e==1) %>% ggplot(aes(x=euc_vec)) + geom_density()
data_prb_a <- data_prb %>% filter(b==8, c==3, e==1)
data_b %>% filter(c==3, e==1) %>% ggplot(aes(x=euc_vec)) + geom_density()
data_c %>% filter(b==8, e==1) %>% ggplot(aes(x=euc_vec)) + geom_density()
data_d %>% filter(b==8, c==3, e==1) %>% ggplot(aes(x=euc_vec)) + geom_density()
data_e %>% filter(b==8, c==3) %>% ggplot(aes(x=euc_vec)) + geom_density()
data_f %>% filter(b==8, c==3, e==1) %>% ggplot(aes(x=euc_vec)) + geom_density()
data_g %>% filter(b==8, c==3, e==1) %>% ggplot(aes(x=euc_vec)) + geom_density()

```


Seeing which conditions appear the most in the top 700 rows with the largest euclidean distance for each parameter. 
...
for (data in data_vec){
  ...
  for(col in 1:6){
    plot <- ggplot(data[1:1000, ], aes(x=data[1:1000, col], y=stat(count))) + geom_bar()
    list_plots[[col]] <- plot
    
  }
  print(summary(data[1:1000, ]))
  print(plot_grid(plotlist=list_plots, nrow=2, ncol=2))
}

ggplot(data_a[1:1000, ], aes(x=d, y=stat(count))) + geom_bar()
```
...
for(col in 1:7){
  if (col == 1){
    large_prb <- data_prb %>% filter(c==3 & e==1) 
    ...
    large_prb <- data_prb %>% filter(a==3 & e==1) 
    print(ggplot(large_prb, aes(x=large_prb[, col], y=x)) 
          + geom_boxplot()
          + xlab(colnames(data_prb[col]))
      + stat_summary(fun = mean, geom = "point", col = "red") 
     ...
     large_prb <- data_prb %>% filter(a==3 & c==3) 
     print(ggplot(large_prb, aes(x=large_prb[, col], y=x)) 
          + geom_boxplot() 
          + xlab(colnames(data_prb[col]))
      + stat_summary(fun = mean, geom = "point", col = "red") 
     ...
     large_prb <- data_prb %>% filter(a==3 & c==3 & e==1)
     print(ggplot(large_prb, aes(x=large_prb[, col], y=x)) 
          + geom_boxplot() 
          + xlab(colnames(data_prb[col]))
      + stat_summary(fun = mean, geom = "point", col = "red") 
      + stat_summary(fun = mean, geom = "text", col = "red",     
               vjust = 1.0, aes(label = paste("Mean:", round(..y.., digits = 1)))))
  }
...
MCSD with all possible combinations of a, c and e.
```{r}
