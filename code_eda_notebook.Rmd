...
CalcEucDist <- function(data, col){
  ...
  for (n in 1:nrow(exp_data)){
    ...
    #saving the data
    out_data <- cbind(exp_data, euc_vec)


    
  }
  
  out_data[out_data$euc_vec != 0, ]
}
...
```{r}
start.time <- Sys.time()
data_a <- CalcEucDist(data_prb, "a")
data_b <- CalcEucDist(data_prb, "b")
data_c <- CalcEucDist(data_prb, "c")
data_d <- CalcEucDist(data_prb, "d")
data_e <- CalcEucDist(data_prb, "e")
data_f <- CalcEucDist(data_prb, "f")
data_g <- CalcEucDist(data_prb, "g")
end.time <- Sys.time()
end.time - start.time
```
